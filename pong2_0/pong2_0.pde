...
//Ball Movement within Pong Table
  ...
  if (keyPressed == true) { //alternate is void keyPressed(){}, always contains the most recent keyboard key stroke
    if (key == 'w') { 
  ...
  if (keyPressed == true & key == CODED) { //alternate is void keyPressed(){}, always contains the most recent keyboard key stroke
    if (keyCode == UP) { //KeyCode is used for UP, DOWN, LEFT, and RIGHT; and ALT, CONTROL, and SHIFT
      if (player[3] >= 5) { //Easier to use && instead of nesting IF Statements
...
textSize(0.05*width);
  text("Player 1:" + score1 + "      player 2:" + score2, (width*1/3)-(0.1*width), height*1/25); //Printing to ration of screen, Player 1 is minusing width of font
  // Note: review of Character escape and this seems to "busy" on the screen
  
  //println ("Score Board is: " + score1 + " Player-1" + "" + score2 + " Player-2");
